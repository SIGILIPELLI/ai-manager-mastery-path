...
Stage gates work as a management tool because they exploit an asymmetry
in how information becomes available during an AI project: uncertainty
about *achievability* is highest at the start and only resolves through
direct empirical work, not through more planning meetings. A software
requirements document can be gotten "mostly right" by thinking harder about
it, because the constraints are usually about human/business logic that a
smart person can reason through in advance. A model's achievable precision
on real data cannot be reasoned to in advance — it can only be discovered by
actually training something and measuring it against held-out examples,
because the relationship between the available data's signal-to-noise ratio
and any model's achievable ceiling is not analytically knowable ahead of
time. This is the mechanical reason "kill criteria" at stages 1–3 aren't
pessimism — they're the only points where the cost of learning "this doesn't
work" is still cheap, because a failed feasibility experiment costs weeks,
while discovering the same ceiling after building full serving
infrastructure in stage 5 costs months and carries sunk-cost pressure to
ship anyway.

The 71%-vs-75% moment in the worked example illustrates the specific trap a
gate is designed to prevent: without a pre-agreed number, "close enough" is
a social judgment made under the exact conditions (deadline pressure, sunk
cost, a team that wants a win) most likely to bias it downward. Writing the
threshold down *before* the proof-of-concept results exist removes the
optimization-after-the-fact problem — nobody can quietly redefine "done" to
match whatever number the model happens to hit, because the metric was
fixed when no one yet knew which way it would cut. This is also why held-out
evaluation data matters mechanically at the Development → Integration gate
specifically: a model's score on data it was tuned against is not evidence
of real-world performance, only of how well it fit that data — the gate
criterion is only meaningful if the evaluation set was truly never used to
make any modeling decision.
