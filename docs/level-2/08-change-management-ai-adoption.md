...
The gap between 84% pilot adoption and 31% rollout adoption is explained by
a selection effect baked into how pilots are usually staffed, not by
anything that changed about the tool. A pilot of volunteers self-selects
for people who already expect to benefit — motivated, often more tolerant
of rough edges, working with the founding team nearby to unblock friction
in real time. That population's usage rate measures "adoption under
conditions of enthusiasm and support," a genuinely different quantity from
"adoption among 220 people who did not choose this, working through a
20-minute video with no live help." Both numbers are real, but they answer
different questions, and generalizing from one to the other is the same
category of error as trusting a vendor's benchmark accuracy: a rate
measured on one distribution doesn't transfer to a materially different one
just because the underlying system didn't change.

The QA-rubric fix mattering more than any product change follows from a
basic incentive-design fact: when a scoring system and a desired behavior
point in opposite directions, the scoring system wins, because it's the
consequence people actually experience every time they act, while "please
use the new tool" is a preference expressed once. An adjuster penalized for
"templated-sounding" language experiences that penalty on every draft they
send, immediately and certainly — while any promised long-run benefit of
adoption is delayed, diffuse, and uncertain. No amount of enablement or
communication changes that asymmetry, because it doesn't touch the actual
mechanism producing the resistance; only changing the rubric itself removes
